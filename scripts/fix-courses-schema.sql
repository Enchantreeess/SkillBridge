-- Complete SkillBridge Database Setup - Fixed Version
-- This script ensures all tables exist with correct schema
...
CREATE TRIGGER handle_updated_at BEFORE UPDATE ON public.credits FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();
...
BEGIN
    NEW.current_level = CASE
        WHEN NEW.total_xp >= 2000 THEN 5
        WHEN NEW.total_xp >= 1000 THEN 4
        WHEN NEW.total_xp >= 500 THEN 3
